...
- `DictationLatencyProbe` is the SwiftPM executable. It depends on the worker package and FluidAudio 0.15.7. Run it with `<Workers/TranscriptionWorker path> <3s.wav> <10s.wav> <30s.wav>`. Its `--vad <directory> <clips...>` mode requires a local FluidAudio `Models/silero-vad/silero-vad-unified-256ms-v6.2.1.mlmodelc` directory and always sets offline mode.
