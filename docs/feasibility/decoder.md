...
Keep a content-based import matrix. Scribe will bundle a minimal, pinned FFmpeg 7.1.1 shared-library build and use its `ffprobe` executable for content-based import probing. The reproducible builder is [`Scripts/build-ffmpeg.sh`](../../Scripts/build-ffmpeg.sh); its SHA-256-pinned upstream archive is `733984395e0dbbe5c046abda2dc49a5544e7e0e1e2366bba849222ae9e3a03b1`. The configuration contains no GPL, version-3-only, nonfree, or external codec options. It enables popular audio and video demuxers plus audio-only decoders (WAV, FLAC, MP3, M4A/AAC/ALAC, AIFF, CAF, Ogg/Opus and Vorbis, Matroska/WebM, MP4/MOV, AVI, MPEG-TS, FLV, ASF/WMA, AC-3, AMR, and related PCM/ADPCM codecs) so video files can be transcribed from their soundtrack. Finder file-reference URLs are resolved to `file:` paths before probing, because `ffprobe` cannot open `/.file/id=…`. Distribution must include [`Native/FFmpeg/NOTICE.md`](../../Native/FFmpeg/NOTICE.md), the LGPL 2.1 text, the corresponding FFmpeg source, and replaceable shared FFmpeg dylibs.
...
`MediaProberTests` generates mono and stereo content in WAV, FLAC, MP3, M4A/AAC, AIFF, CAF, and Ogg/Opus, plus Matroska, WebM, and MP4, deliberately gives every output a misleading or absent suffix, checks 44.1/48 kHz metadata, exercises Unicode and spaces in paths, probes Finder file-reference URLs, and checks that random bytes named `.m4a` produce a structured corrupt error. The tests use a development FFmpeg with MP3 and Opus encoders only to synthesize test inputs; the production probe uses the pinned bundled `ffprobe`.
